...
Command-line tool that time-syncs a telemetry log (CSV) to a camera video (MP4) using IMU cross-correlation. Current support targets GoPro MP4 (GPMF) and AiM CSV. It estimates the time offset by comparing motion patterns and prints a RaceRender-friendly offset instruction.

**How It Works**
1. Extract GoPro IMU (gyro/accel) from MP4 using `pygpmf-oz` and normalize timestamps to start at 0.0 seconds.
2. Parse AiM CSV, detect delimiter and column names, and convert time to seconds-from-start.
...
4. Run in a directory with exactly one `.mp4` and one `.csv`: `imu_video_sync`

**Usage**
```
imu_video_sync
imu_video_sync --mp4 session.mp4 --csv aim.csv
...
imu_video_sync --aim-time-col "Time" --aim-gyro-cols "GyroX,GyroY,GyroZ"
imu_video_sync --write-shifted-csv
imu_video_sync --plot
```

**Outputs (Printed)**
- Selected GoPro signal and inferred sample rate.
- Selected AiM signal and inferred sample rate.
- **Sync Summary** with:
...
**Outputs (Files)**
- `gopro_imu.csv` if `--write-gopro-csv`
  - Columns: `time_s, gx, gy, gz, ax, ay, az` (whichever are available).
  - Useful for debugging or external analysis.
- `aim_shifted.csv` if `--write-shifted-csv`
  - AiM CSV with timestamps shifted by the computed lag.
...
**Design Principles**
- One reliable path: uses `pygpmf-oz` for GoPro telemetry (no alternate backends).
...
- The AiM shifted CSV overwrites the time column with seconds-from-start values plus the computed lag.
