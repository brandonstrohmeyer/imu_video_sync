...
`imu-video-sync` aligns a GoPro video with an AiM CSV log by comparing motion patterns from each device's IMU (gyro/accelerometer).

## High-level Flow
1. Read GoPro telemetry from the MP4 and extract gyro/accel samples with timestamps.
2. Read AiM CSV and detect time plus IMU columns.
...
The tool does not rely on one window or a lap length. It scans many fixed-length windows across the AiM log, scores each alignment, and then builds a consensus from the best windows.

## Outputs
The CLI prints a sync summary and a RaceRender-friendly offset line. Optional outputs include a GoPro IMU CSV, a shifted AiM CSV, and a diagnostic plot.
